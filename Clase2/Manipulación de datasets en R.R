...
Proveedoreg  <-  data.frame(Region = c("CABA", "CABA", "Santa Fe", "Córdoba","","San Luis",""))
...
#A modo de ejemplo tambiÃ©n tenemos ventas que no pertenecen a ningÃºn cliente, 
#por ejemplo el registro # 4 que pertenecerÃ?a a un Cliente 5 inexistente
...
#y obviamente las ventas del cliente inexistente 5 tampoco aparecerán


# LEFT JOIN
# Un clásico left join donde se muestra todos los registros del data.frame izquierdo (x) 
...
# y también reemplazamos los valores NA de las columnas que correspondan.
...
summary(customer)
customer2 <- read.csv("Customer.csv", stringsAsFactors=TRUE)
summary(customer2)
...
library(dplyr)
# Filtrar la base para los pasajeros con edad >= 40 años
...
# Es una sintaxis en cadena, de forma que el operador %> % coge el output ('la salida') de una sentencia de código y la convierte en el input ('el argumento') de una nueva sentencia.
