...
#-----------------------
# *POR UN TEMA DE RENDIMINETO DE LA PC, TOMO UNA MUESTRA DE n = 10000
#--------------------------------------------------------------------
...
ggplot(data=filter(cli_sample, CL_TECNO!='99-NOSEGM' ))+
  geom_histogram(aes(x=MONTO_TOTAL), fill="blue", alpha=.3,bins=70)

ggplot(data=filter(cli_sample, CL_TECNO!='99-NOSEGM' ))+
  geom_histogram(aes(x=CANT_RECARGAS), fill="red", alpha=.3,bins=70)
...
#
#-Vemos que todos los segmentos están relativamente alineados, por lo tanto
...
# 1.	Hay un 38.5% de clientes ACTIVE BASE o REJOINER que no realizaron cargas 
# significativas (menos de 3) en los meses estudiados. 
# 
# 2.	Más allá del segmento que se estudie, encontramos una relación lineal positiva
...
#
# 4. los segmentos estudiados tienen una distribución asimétricas entre si.
# 
# 5.	EL 21.6% de los clientes usa el canal tecnológico para realizar recargas, 
